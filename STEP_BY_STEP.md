...
**Cause:** File name with spaces/special characters
...
old_blob = bucket.blob("raw_data/file with spaces.csv")
new_blob_name = "raw_data/file_without_spaces.csv"
...
# Remove ALL decorative comments (lines with ====)
...
**Fix:** Already fixed in requirements.txt. If it shows up:
```bash
...
**This is normal!** Embedding creation takes time:
- 25 questions: ~2 minutes
- 100 questions: ~5 minutes
- 1000 questions: ~30 minutes

**Tip:** Once created, embeddings are saved in the bucket. Later runs are fast (just load)!
